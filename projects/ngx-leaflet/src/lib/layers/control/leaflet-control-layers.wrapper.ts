...
export class LeafletControlLayersWrapper {
    ...
    applyBaseLayerChanges(
        changes: KeyValueChanges<string, Layer>,
    ): LeafletControlLayersChanges {
        let results: LeafletControlLayersChanges =
            new LeafletControlLayersChanges();

        if (null != this.layersControl) {
            results = this.applyChanges(
                changes,
    ...
    private applyChanges(
        changes: KeyValueChanges<string, Layer>,
        addFn: (layer: Layer, name: string) => void,
    ): LeafletControlLayersChanges {
        const results: LeafletControlLayersChanges =
            new LeafletControlLayersChanges();
