...
export class LeafletLayerDirective implements OnChanges, OnDestroy, OnInit {
    ...
    private addLayerEventListeners(l: Layer) {
        this.onAddLayerHandler = (e: LeafletEvent) =>
            LeafletUtil.handleEvent(this.zone, this.onAdd, e);
        l.on('add', this.onAddLayerHandler);

        this.onRemoveLayerHandler = (e: LeafletEvent) =>
            LeafletUtil.handleEvent(this.zone, this.onRemove, e);
